...
void jeu(Console*pConsole)
 {
    ...
    bool quit =  false;
     ...
     ///Initialisation d'un tab de pions, les 23 premiers sont noirs, les 32 autres blancs.
    Pion pions[NBPIONS];
    ...
    bool c=0;
    int pionblancselect=34;
    int pionnoirsselect=2;

    int tour= NOIR; ///Les noirs commencent à jouer

    pions[0].setligne(3);
    pions[0].setcolonne(3);
    pions[0].setvisible(1);
    tab[3][3] = 2;


    pions[1].setligne(4);
    pions[1].setcolonne(4);
    pions[1].setvisible(1);
    tab[4][4] = 2;

    pions[32].setligne(3);
    pions[32].setcolonne (4);
    pions[32].setvisible(1);
    tab[3][4] = 1;


    pions[33].setligne(4);
    pions[33].setcolonne(3);
    pions[33].setvisible(1);
    tab[4][3] = 1;
    ...
    while(!quit)
    {
        ...
        //else {///BOUCLE DE JEU
            ...
            if ( (tab[(ligne-6)/2][(colonne-13)/3]==0) && (adjacence(ligne, colonne, tab, tour)))
                /*&&(adjacence(ligne,colonne,tab))*/
            {
...
bool gestion_curseur(Console *pConsole, int &ligne, int &colonne, int**tab )
 {
    ...
    if (caractere=='s') ///Sauvegarde de la parie
        {
